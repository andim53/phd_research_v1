...
a_guess = 3.92  # Initial guess for FCC Platinum (Å)
kpts = (6, 6, 6)
...
for scaling in np.linspace(0.95, 1.05, 5):
    ...
    # Create the cubic FCC bulk structure (4 atoms in the conventional cell)
    pt_bulk = bulk("Pt", "fcc", a=a_current, cubic=True)
    ...
    calc = GPAW(
        ...
        # maxiter=100,
        txt=f"{dir_save}/pt_{a_current:.2f}.txt",  # Separates logs for each step
    )
    pt_bulk.calc = calc
    
    # Calculate total energy and volume
    energy = pt_bulk.get_potential_energy()
    volume = pt_bulk.get_volume()
