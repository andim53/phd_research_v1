...
a_guess = 3.16  # Initial guess for BCC Tungsten (Å)
kpts = (6, 6, 6)
...
for scaling in np.linspace(0.95, 1.05, 5):
    ...
    w_bulk = bulk("W", "bcc", a=a_current, cubic=True)
    ...
    calc = GPAW(
        ...
        # maxiter=100,
        txt=f"{dir_save}/w_{a_current:.2f}.txt",  # Separates logs for each step
    )
    w_bulk.calc = calc
    
    # Calculate total energy and volume
    energy = w_bulk.get_potential_energy()
    volume = w_bulk.get_volume()
...
output_filename = "opt_w.txt"
with open(output_filename, "w") as f:
    f.write("=== Tungsten Bulk Lattice Optimization Summary ===\n")
